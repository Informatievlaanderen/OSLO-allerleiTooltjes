-- "All Tags on Classifiers (Classes, DataTypes, Enumerations), Attributes and Connectors in Diagrams" (Diagram_ID in <Search Term> - meerdere Diagram_IDs gescheiden door komma's toegelaten)
-- NOTE: Scope is niet beperkt tot een package of een branh

-- Class, Datatype, Enumeration (+ andere elementen indien aanwezig)
...
UNION ALL
-- Connector - direction To Destination --> forward
...
WHERE
	...
	AND t_diagram.Diagram_ID IN (<Search Term>)
UNION ALL
-- Connector - direction To Source --> reverse
...
WHERE
	...
	AND t_diagramlinks.Hidden = false
	AND t_diagram.Diagram_ID IN (<Search Term>)
-- Connector - direction No indicator (= bi-directional) --> not (yet) supported
